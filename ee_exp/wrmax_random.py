...
class WrmaxRandom(object):
    """
    Wrmax class
    """

    def  __init__(self, candidates, env, weight=0.3, dropout_prob=1.0, batch_size=64, replay_memory_size=10000):
        ...
        self._sample_fake = dict.fromkeys([i for i in range(self._candidate_number)], 1)
        self._env = env
    ...
    def update_one_sample(self, context_feature, nid, reward, model_type='', train_set=''):
        ...
        for nid, doc in enumerate(self._candidates):
          increase_num = math.log(self._count) * self._weight - self._sample_fake[nid]
          if random.random() < (increase_num - int(increase_num)):
          ...
          if increase_num > 0:
            ...
            self.init(fake_data)
            self._sample_fake[nid] += increase_num
          # print 'nid:', nid, self._sample_fake[nid], self._count, increase_num
