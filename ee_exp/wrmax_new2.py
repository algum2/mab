...
class WrmaxNew2(object):
    """
    Wrmax class
    """

    def  __init__(self, candidates, env, weight=0.3, fake_prob=0.5, dropout_prob=1.0, batch_size=64, replay_memory_size=10000):
    ...
    def update_one_sample(self, context_feature, nid, reward, model_type='', train_set=''):
        ...
        train_set = self._replay_memory.draw_by_all(512)
        num = train_set[0].shape[0]
        #print train_set
        if not train_set:
          return
        #train_set = self._replay_memory.draw_by_all(self._replay_memory_size)
        prob = train_set[0].shape[0] / self._replay_memory.get_size()
        for nid, doc in enumerate(self._candidates):
          tmp = self._weight * math.sqrt(math.log(self._count))
          under = math.sqrt(self._sample_ture[nid]) - tmp
          if 0 >= under: 
            if random.random < self._fake_prob:
              increase_num = 1
            else:
              increase_num = 0
          else:
            increase_num = (tmp * self._sample_ture[nid] / under) * prob
            if random.random() < (increase_num % 1):
              increase_num = int(increase_num) + 1
            else:
        ...
        print >> sys.stderr, self._count, (train_set[0].shape)[0] - num, num
